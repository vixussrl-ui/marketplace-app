...
// Helper function to calculate Base Price (same logic as Print costs page)
...
export default function MarketplacePriceSetPage() {
  ...
  const calculateMarketplacePrice = useCallback((record, marketplace) => {
    ...
    // Formula: preț_final = Base Price + (Base Price * commission / 100) + transportCost
    // Sau: preț_final = Base Price + comision + transport
  ...
  const columns = useMemo(() => {
    const baseColumns = [
      ...
      {
        title: (
          <Tooltip title="The base selling price to achieve the target print rate. Calculated as: (Target Print Rate (RON/h) / Printed items/hour) + Material Cost + Electricity Cost (RON) + Packaging Costs (RON). For multiple parts products, this is the sum of base prices for all parts plus packaging cost.">
        ...
        >
          <div style={{ marginBottom: '16px', padding: '12px', background: theme.COLORS.primaryLight, borderRadius: theme.RADIUS.md }}>
            <Text type="secondary" style={{ fontSize: '14px' }}>
              <strong>Formula:</strong> Marketplace Price = Base Price + (Base Price × Commission %) + Transport Cost
            </Text>
          </div>
        ...
        >
          ...
          >
            ...
            <Form.Item
              ...
              label="Base Price (RON)"
              rules={[
                { required: true, message: 'Please enter base price' },
                { type: 'number', min: 0, message: 'Base price must be positive' }
              ]}
            >
              <InputNumber
                min={0}
